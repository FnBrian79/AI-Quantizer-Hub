 ...
 * Auto-migrates v1 plain-text entry to encrypted v2 on first run.
 */
export async function loadSecure<T>(fallback: T): Promise<T> {
  try {
    ...
    // Corrupt data, wrong key, or parse error → safe fallback
    return fallback;
  }
}
 ...
 * Reserved for a future "Reset Settings" button.
 */
